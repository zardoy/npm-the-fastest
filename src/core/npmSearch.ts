...
export type NpmSearchResult = Array<
    SetOptional<Pick<AlgoliaSearchResultItem, 'name' | 'version' | 'humanDownloadsLast30Days' | 'description' | 'bin'>, 'bin' | 'humanDownloadsLast30Days'> & {
        owner: string
...
export const performAlgoliaSearch = async (search: string): Promise<NpmSearchResult> => {
    ...
    const makeAttributesToRetrieve = <T extends (keyof AlgoliaSearchResultItem)[]>(t: T) => t
